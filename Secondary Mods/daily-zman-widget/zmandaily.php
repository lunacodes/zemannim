	...
	*/
	...
	function widget($args, $instance) {
		...
		/* JSON get lat/long from zip */
        	...
        	// luna debug
        	$debug_x_forwarded_for = $_SERVER['HTTP_X_FORWARDED_FOR'];
        	$debug_remote_addr =  $_SERVER['REMOTE_ADDR'];
        	var_dump($debug_x_forwarded_for);
        	var_dump($debug_remote_addr);
        	echo("IP is: $ip");
        	...
        	$lat = $data['8'];
        	$long = $data['9'];
        	// luna debug - check if manual entry is working
        	// $lat = 30.5595;
        	// $long = 22.9375;
